...
class _GetStoreInputState extends State<GetStoreInput> {
  ...
  double pitch = 1.0;
  double rate = 0.5;
  bool _isConfirming = false; // 편의점 이름 입력 후 확인 단계 표시

  @override
  void initState() {
    super.initState();
    _speak("방문할 편의점을 말씀해주세요!");
  }
  ...
  void _updateRecognizedText(String text) {
    setState(() {
      _storeName = text;
      _isConfirming = true;
      _confirmationText = '$text 라고 말씀하셨으면 터치 패드를 한번 터치, 아니라면 두번 터치하세요.';
